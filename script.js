...
const brand = window.BusinessBrand || { businessName: "Seu negocio", whatsapp: "", policy: { cancellationHours: 2 } };
let shop = null;
let services = [];
...
document.querySelector("#booking-form").addEventListener("submit", async (event) => { event.preventDefault(); if (!state.time) { summary.textContent = "Escolha uma data e horario antes de continuar."; return; } const form = new FormData(event.currentTarget), payload = { name: form.get("name"), phone: form.get("phone"), serviceId: state.service.id, date: dateValue(state.date), time: state.time, policyAccepted: true }; let data, response; try { if (demoMode) data = { booking: DemoStore.createBooking(payload), pix: { pixRecipient: brand.businessName, pixKey: brand.pixKey, depositCents: brand.depositCents, whatsapp: brand.whatsapp } }; else { response = await fetch("/api/bookings", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) }); data = await response.json(); if (!response.ok) throw new Error(data.error); } } catch (error) { summary.textContent = error.message; await render(); return; } const booking = data.booking; shop = data.pix; document.querySelector("#success-message").textContent = `Reserva ${booking.code} criada para ${dateText(state.date)} as ${booking.time}. Faca o sinal para ${brand.businessName} confirmar.`; document.querySelector("#pix-details").hidden = false; document.querySelector("#pix-value").textContent = money(shop.depositCents / 100); document.querySelector("#pix-recipient").textContent = `Recebedor: ${shop.pixRecipient}`; document.querySelector("#pix-key").textContent = shop.pixKey; document.querySelector("#success-dialog").showModal(); event.currentTarget.reset(); state.time = null; await render(); });
...
function renderReservation(booking) {
  ...
  const appointment = new Date(`${booking.date}T${booking.time}:00`), cancellationHours = brand.policy?.cancellationHours ?? 2, canCancel = !["cancelado", "concluido", "falta"].includes(booking.status) && appointment.getTime() - Date.now() >= cancellationHours * 60 * 60 * 1000;
  ...
  feedback.textContent = canCancel ? `Cancelamentos sao permitidos com pelo menos ${cancellationHours} horas de antecedencia.` : "Esta reserva nao pode mais ser cancelada online."; lucide.createIcons();
}
